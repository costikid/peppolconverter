...
public class ExtractionService {
    ...
    public ExtractedInvoice extract(MultipartFile file) throws IOException {
        try (PDDocument document = PDDocument.load(file.getInputStream(),
            ...
            log.error("Error extracting PDF: {}", e.getMessage(), e);
            throw e;
        }
    }
    ...
    private ExtractedInvoice parseFreeAgentText(String text) {
        ...
        log.info("Parsed invoice: number={}, seller={}, buyer={}",
    ...
    private void parseSellerAndBuyer(String[] lines, ExtractedInvoice invoice) {
        // FreeAgent layout from extracted text:
        // Lines 0-3: Seller details (***REMOVED***, ***REMOVED***, ***REMOVED***, ***REMOVED***)
        // Line 4: "Doug Andrews INVOICE 002"
        // Line 5: Buyer name (Globe 2 Ltd.)
        // Lines 6-9: Buyer address with dates mixed in (24 April 2026, 42 Coventry Road, Payment due by 01 May 2026, Market Harborough, Leicestershire LE16 9BZ)

        ExtractedInvoice.Party seller = new ExtractedInvoice.Party();
        ExtractedInvoice.Party buyer = new ExtractedInvoice.Party();

        // Parse seller (lines 0-3)
        int sellerIdx = 0;
        ...
        // Find buyer start (after INVOICE line)
        int buyerStart = -1;
        for (int i = 0; i < lines.length; i++) {
            String line = lines[i].trim();
            if (line.contains("INVOICE")) {
                // Buyer is on the next non-empty line after INVOICE
        ...
        // Parse buyer address - skip date lines and "Payment due" lines
        if (buyerStart >= 0) {
            int i = buyerStart;
            while (i < lines.length) {
                ...
                // Skip date lines and "Payment due" lines
    ...
    private void parseLineItems(String[] lines, ExtractedInvoice invoice) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (line.contains("Quantity") && line.contains("Details") && line.contains("Unit Price")) {
                // Header found, parse lines below
                for (int j = i + 1; j < lines.length; j++) {
    ...
    private ExtractedInvoice.LineItem parseLineItem(String line, int lineNumber) {
        // Format from extracted text: "1 Emma's referral 20.00 20.00"
        // Qty Description UnitPrice Subtotal
        try {
            String[] parts = line.split("\\s+");
            if (parts.length < 4) return null;

            // Find the numeric values at the end
            BigDecimal qty = null;
            ...
            // Description is everything between qty and the prices
            int qtyIdx = -1;
            ...
            item.setUnitCode("EA"); // Default unit code
    ...
    private void parsePaymentDetails(String[] lines, ExtractedInvoice invoice) {
        ...
        // Bank name is usually the first line before Sort Code
